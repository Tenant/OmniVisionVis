...
void OmniVision::showVelo()
{
	...
	if (_useBGR) {
		_mono_velo = _monoData.img.clone();
		equalize_HSV_V(_mono_velo);
	}
	else {
	...
	{
		if (_useBGR) {
			_pano_velo = _ladybug.getCurrentData().img.clone();
...
void OmniVision::equalize_HSV_V(cv::Mat& srcImg)
{
	Mat HSV;
	vector<Mat> hsvSplit;
	...
	merge(hsvSplit, HSV);
	cvtColor(HSV, srcImg, cv::COLOR_HSV2BGR);
}